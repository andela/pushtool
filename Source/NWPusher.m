...
{
    NWPusher *PushTool = [[NWPusher alloc] init];
    return identity && [PushTool connectWithIdentity:identity environment:environment error:error] ? PushTool : nil;
}
...
{
    NWPusher *PushTool = [[NWPusher alloc] init];
    return data && [PushTool connectWithPKCS12Data:data password:password environment:environment error:error] ? PushTool : nil;
}
