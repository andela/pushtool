...
- (void)updateCertificatePopup
{
    ...
    for (NSArray *pair in _certificateIdentityPairs) {
        ...
        NSString *type = nil; //descriptionForCertType(certType);
        ...
        [_certificatePopup addItemWithTitle:[NSString stringWithFormat:@"%@%@ (%@ %@)%@%@", hasIdentity ? @"imported: " : @"", summary, type, nil /*descriptionForEnvironentOptions(environmentOptions)*/, expire, suffix]];
        [suffix appendString:@" "];
    }
...
- (NSString *)identifierWithCertificate:(NWCertificateRef)certificate
{
    NWEnvironmentOptions environmentOptions = [NWSecTools environmentOptionsForCertificate:certificate];
    NSString *summary = [NWSecTools summaryWithCertificate:certificate];
    return summary ? [NSString stringWithFormat:@"%@-%@", summary, nil /*descriptionForEnvironentOptions(environmentOptions)*/] : nil;
}
...
- (NSURL *)configFileURL
{
    ...
    NSURL *configURL = [libraryURL URLByAppendingPathComponent:@"PushTool" isDirectory:YES];
